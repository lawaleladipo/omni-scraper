...
def crawl_cmd(seeds, max_depth, max_pages, concurrency, no_tor, fmt):
    ...
    crawler = AsyncWebCrawler(seeds=seeds, concurrency=concurrency, max_depth=max_depth, max_pages=max_pages, use_tor=not no_tor)
    try:
...
def scrape_cmd(url, save_html, no_tor, fmt):
    ...
    if save_html:
        ...
        html_path = Path(config.get('output.directory','data/outputs')) / f"{safe_name}.html"
        html_path.parent.mkdir(parents=True, exist_ok=True)
    try:
        result = asyncio.run(scraper.scrape(url, save_html=save_html, html_path=str(html_path) if html_path else None))
        output_handler.format = fmt
