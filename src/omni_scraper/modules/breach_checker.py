...
class BreachChecker:
    ...
    def _headers(self):
        headers = {'User-Agent': self.config.get('user_agent', 'OmniScraper/0.3'), 'Accept': 'application/json'}
        if self.api_key:
    ...
    def check_email(self, email):
        ...
        params = {'term': email, 'buckets': 1, 'terminate': ['email', 'domain'], 'limit': 10, 'timeout': 5}
        try:
            ...
            return [{'source': l.get('title'), 'date': l.get('date'), 'snippet': l.get('content', '')[:200]} for l in leaks]
