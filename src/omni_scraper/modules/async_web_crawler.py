...
class AsyncWebCrawler:
    def __init__(self, seeds=None, concurrency=None, max_depth=None, max_pages=None, use_tor=True):
    ...
    async def _worker(self, session, queue):
        while not queue.empty() and len(self.visited) < self.max_pages and not self._stop_flag.is_set():
            try:
            ...
            if html:
                ...
                self.results.append({'url': url, 'depth': depth, 'emails': emails, 'snippet': snippet})
    ...
    async def run(self, seeds=None):
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            workers = [asyncio.create_task(self._worker(session, queue)) for _ in range(self.concurrency)]
            await queue.join()
            self._stop_flag.set()
            for w in workers:
                w.cancel()
            await asyncio.gather(*workers, return_exceptions=True)
        logger.info(f"Crawl complete: pages={len(self.results)} visited={len(self.visited)}")
        return self.results
